...
def _get_fpn_anchor_box(width: int, height: int):
    ...
    for _ in range(4):  # P4 through P7
        shapes.append(
            (int(np.ceil(shapes[-1][0] / 2)), int(np.ceil(shapes[-1][1] / 2))))
...
class AnchorBox(NumpyOp):
    ...
    def __init__(self, width, height, inputs, outputs, mode=None):
        ...
        self.anchorbox, _ = _get_fpn_anchor_box(
            width, height)  # anchorbox is #num_anchor x 4
    ...
    def _generate_target(self, bbox):
        ...
        ious = self._get_iou(object_boxes,
                             self.anchorbox)  # num_obj x num_anchor
        ...
        cls_gt = np.int32([label[idx]
                           for idx in anchor_to_obj_idx])  # num_anchor x 1
        ...
        return np.array([dx1, dy1, dwidth, dheight,
                         cls_gt]).T  # num_anchor x 5
    ...
    def _get_iou(boxes1, boxes2):
        ...
        inter_area = np.maximum((xmax - xmin + 1), 0) * np.maximum(
            (ymax - ymin + 1), 0)
        area1 = (w1 + 1) * (h1 + 1)
        area2 = (w2 + 1) * (h2 + 1)
...
class FocalLoss(TensorOp):
    ...
    def forward(self, data, state):
        ...
        for idx in range(batch_size):
            ...
            single_focal_loss = self.focal_loss(single_cls_gt, single_cls_pred)
...
class PredictBox(TensorOp):
    ...
    def __init__(self,
        ...
        self.all_anchors, self.num_anchors_per_level = _get_fpn_anchor_box(
    ...
    def forward(self, data, state):
        ...
        labels_pred, scores_pred = tf.argmax(cls_pred,
                                             axis=-1), tf.reduce_max(cls_pred,
                                                                     axis=-1)
        # loc_pred -> loc_abs
        x1_abs = loc_pred[..., 0] * self.all_anchors[...,
                                                     2] + self.all_anchors[...,
                                                                           0]
        y1_abs = loc_pred[..., 1] * self.all_anchors[...,
                                                     3] + self.all_anchors[...,
                                                                           1]
        ...
        for idx in range(batch_size):
            ...
            boxes_pred_single = tf.stack(
                [y1_abs[idx], x1_abs[idx], y2_abs[idx], x2_abs[idx]], axis=-1)
            # iterate over each pyramid to select top 1000 anchor boxes
            start = 0
            top_idx = []
            for num_anchors_fpn_level in self.num_anchors_per_level:
                fpn_scores = scores_pred_single[start:start +
                                                num_anchors_fpn_level]
                selected_index = tf.math.top_k(
                    fpn_scores,
                    min(self.select_top_k, int(num_anchors_fpn_level))).indices
            ...
            nms_keep = tf.image.non_max_suppression(
                tf.gather(boxes_pred_single, top_idx),
                tf.gather(scores_pred_single, top_idx), self.nms_max_outputs)
            ...
            results_single[0] = tf.clip_by_value(
                results_single[0],
                clip_value_min=0,
                clip_value_max=self.input_shape[1])
            results_single[1] = tf.clip_by_value(
                results_single[1],
                clip_value_min=0,
                clip_value_max=self.input_shape[0])
            results_single[2] = tf.clip_by_value(
                results_single[2],
                clip_value_min=0,
                clip_value_max=self.input_shape[1] - results_single[0])
            results_single[3] = tf.clip_by_value(
                results_single[3],
                clip_value_min=0,
                clip_value_max=self.input_shape[0] - results_single[1])
            # mark the select as 0 for any anchorbox with score lower than threshold
            results_single[-1] = tf.where(
                results_single[-2] > self.score_threshold, results_single[-1],
